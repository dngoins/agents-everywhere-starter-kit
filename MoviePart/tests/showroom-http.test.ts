...
test("real fixture HTTP: one-time pair, consent, quiet references, corrections, frozen movie, browser playback and calendar", { timeout: 30000 }, async t => {
  ...
  const calendar: ShowroomCalendar = {
    ...
    confirm: async () => { calendarCalls++; return { status: "created", eventId: "fixture-calendar", invitationsRequested: true }; },
  };
  ...
  assert.equal(calendarCalls, 1, "ending the session cannot create or cancel a confirmed appointment");
