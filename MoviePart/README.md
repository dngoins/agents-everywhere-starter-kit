...
When Google Veo fails before returning an operation ID or rejects a completed clip during continuity review, the worker automatically makes at most two new Veo submissions while preserving approved storyboard work. A missing operation ID means the prior request may also have been accepted and charged. If both Veo replacements fail, the worker automatically switches to a new Sora 2 Pro sequence from the approved hero storyboard image. Sora runs its existing product-only person/face safety check before submission, creates a complete Sora sequence rather than mixing providers, and preserves the original Veo operations as provenance. If Sora cannot accept the reference or complete the sequence, the job stops honestly; image-motion fallback remains operator-approved and is never automatic.
