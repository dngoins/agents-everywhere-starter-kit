...
When a completed Google Veo clip fails continuity review, ordinary retry does not resubmit it. The operator may explicitly authorize up to two replacement clips; each authorization can incur another provider charge and preserves approved storyboard work and prior clips. The operator may instead explicitly authorize a new Sora 2 Pro sequence from the approved hero storyboard image. Sora runs its existing product-only person/face safety check before submission, creates a complete Sora sequence rather than mixing providers, and preserves the original Veo operations as provenance. If both Veo replacements fail, the operator may explicitly finish the saved plan as **Image motion only**. Neither fallback is automatic.
