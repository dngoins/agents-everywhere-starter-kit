...
export class LiveToolDispatcher {
  constructor({ send, onTool, isCurrent, onError, onDrained, allowedTools = [] } = {}) {
  ...
  async _result(call) {
    if (!this.allowedTools.has(call.name)) return { ok: false, error: 'This tool is not available to the voice model.' };
    let args;
    try {
...
export class LiveVoice {
  ...
  async connect({ identity = {}, microphone } = {}) {
    ...
    try {
      ...
      if (!object(identity)) throw new VoiceFailure('A valid application session identity is required for voice.');
      if (!options.sessionFactory && !options.sessionEndpoint) throw new VoiceFailure('Configure a voice session factory or endpoint.');
      ...
      for (const track of tracks) {
        ...
        clone.enabled = !this._paused && !this._microphoneMuted;
      ...
      if (options.sessionFactory) {
        result = await abortable(options.sessionFactory({ sdp, identity, signal }), signal);
      } else {
        const fetchSession = options.fetch || globalThis.fetch.bind(globalThis);
        const response = await abortable(fetchSession(options.sessionEndpoint, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...identity, sdp }), signal,
        }), signal);
        this._assert(run);
        try { result = await abortable(response.json(), signal); } catch {
          ...
          if (!response.ok) {
            ...
            throw new VoiceFailure(`Voice server request failed${status}. ${options.errorHints?.server || 'Check the voice server and reconnect.'}`);
          }
      ...
      const safe = error instanceof VoiceFailure ? error
        : new VoiceFailure(options.errorHints?.connect || 'Could not connect voice. Check microphone permissions, browser WebRTC support, and the server.');
      this._fail(run, safe);
      throw safe;
    }
  }
  ...
  _tryReady(run) {
    ...
    if (this._active(run) && (this._paused || this._microphoneMuted)) this._send(run, { type: 'session.input_audio.mute' });
  }
  ...
  setPlaybackPaused(paused) {
    ...
    for (const track of run?.tracks || []) track.enabled = !this._paused && !this._microphoneMuted;
    if (!run || this.audioElement?.srcObject === run.remote) {
      if (this.audioElement) this.audioElement.muted = this._paused;
    }
    if (run?.ready) this._send(run, { type: (this._paused || this._microphoneMuted) ? 'session.input_audio.mute' : 'session.input_audio.unmute' });
  ...
  greet() {
    ...
    if (!this.options.greetingContext?.trim()) return false;
    this._greeted = this._command({ type: 'session.instructions.append', content: appendContent(this.options.greetingContext) });
    return this._greeted;
  }

  movieReady() {
    if (this._movieReady) return false;
    if (!this.options.readyContext?.trim()) return false;
    this._movieReady = this._command({ type: 'session.thinking.append', content: appendContent(this.options.readyContext) });
