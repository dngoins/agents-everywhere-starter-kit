...
| POST `/v1/sessions/{id}/assets` | session bearer | raw PNG/JPEG bytes / existing `AssetUploaded` |
...
disabled capability 503. Voice setup must issue a short-lived OpenAI client
credential using existing voice settings; it is not a long-lived provider key.
...
`motion_requested` stages a bounded `MotionIntent`. None executes a side effect
