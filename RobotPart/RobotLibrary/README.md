...
2. Discovery filters include that service and name prefixes `PadBot` / `padbot` (alternatives, not combined requirements).
3. Commands are UTF-8 strings. Speed is a separate command: never prefix a motion token with a speed number (`2X1` is not the observed protocol).
...
5. Firmware framing varies: `raw` sends `X1`, `mn` sends `mX1n`, `pq` sends `pX1q`. Default `auto` sends all three sequentially; it does **not** detect firmware or negotiate the correct mode. A robot may accept more than one variant.
...
7. Connection initialization sends the selected speed (default medium), infrared query, and, in auto mode, info query. It never sends a movement command. Set `initialize: false` to omit these writes. Unlike the app's best-effort infrared probe, any completely failed initialization command rejects library connection and closes GATT.
...
| `left()` / `right()` | `X6` / `X7` | Turn left / right |
...
| `setSpeed("fast")` or `setSpeed(3)` | `W` | Fast |
| `initializeSpeed()` | `]` | Observed firmware speed/setup token; not a speed selection |
| `queryBattery()` | `?` | Request battery/charge data |
| `queryInfrared()` | `&` | Request obstacle/infrared data |
| `queryInfo()` | `:` | Request robot info/version |
...
| `protocolMode` | `"auto"` | `"raw"`, `"mn"`, `"pq"`, or `"auto"` |
| `speed` | `"medium"` | `"low"`, `"medium"`, `"fast"` or numeric `1`, `2`, `3` |
...
- `robot.connected`, `robot.device`, `robot.speed`, `robot.lastCommand` — read-only accessors. `speed` is the last successfully written selection (or constructor default), not measured speed. `lastCommand` resets on disconnect.
- `robot.connectionInfo` — snapshot with `connected`, `deviceId`, `deviceName`, `serviceUuid`, `writeUuids`, `notifyUuid`, and `protocolMode`.
...
`await robot.setSpeed(value)` changes the speed separately, including during a continuous hold, without cancelling movement. Queries also preserve a hold. Only the three observed speed levels are supported; there is no invented percentage-speed API.
